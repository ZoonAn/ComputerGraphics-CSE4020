...
def getBSpline(t, p0, p1, p2, p3): 
    return (((1 - t) ** 3) * p0
     + (3 * (t ** 3) - 6 * (t ** 2) + 4) * p1
     + (-3 * (t ** 3) + 3 * (t ** 2) + 3 * t + 1) * p2
     + (t ** 3) * p3) / 6

def setCowRotation(directionVec):
    global cowRotationPos
    ...
    cowRotationPos[:3, :3] = (Ry@Rx@Rz).T

def display():
    global cameraIndex, cow2wld, inputState, inputLocation, animationStartTime, inputDone, cowRotationPos
    ...
    if -1 < inputState and inputState < 6:
        for i in inputLocation:
            drawCow(i, True)
    elif inputState == 6:
        animationTime = glfw.get_time() - animationStartTime

        cowSplinePos = np.zeros((4, 4))

        temp = animationTime % 6

        if temp < 6 and animationTime < 18:
            cowSplinePos = getBSpline(float(animationTime) - int(animationTime),
             inputLocation[int(temp) - 1], inputLocation[int(temp)], inputLocation[int(temp) - 5], inputLocation[int(temp) - 4])

        else:
            cow2wld = cowRotationPos
            inputState = -1
            inputLocation.clear()
            inputDone = False
            glFlush()
        ...
        setCowRotation(normalize(getTranslation(cowSplinePos) - getTranslation(cowRotationPos)))
        setTranslation(cowRotationPos, getTranslation(cowSplinePos))
        drawCow(cowRotationPos, False)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, inputState, animationStartTime, inputDone, cowRotationPos
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox:
                ...
                print("inputState %d\n" % inputState)
                if inputState > 0:
                    inputLocation.append(cow2wld.copy())
                    if inputState == 6:
                        animationStartTime = glfw.get_time()
                        inputDone = True
                        cowRotationPos = inputLocation[0].copy()
                        isDrag = 0
