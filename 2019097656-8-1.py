...
gCamAng = 0.
gCamHeight = 1.
angAlpha = np.radians(0)
angBeta = np.radians(0)
angGamma = np.radians(0)
...
def render():
    global gCamAng, gCamHeight, angAlpha, angBeta, angGamma
    ...
    M = np.identity(4)
    Rx = np.array([[np.cos(angAlpha), -np.sin(angAlpha), 0],
                   [np.sin(angAlpha), np.cos(angAlpha), 0],
                   [0,0,1]])
    Ry = np.array([[np.cos(angBeta), 0, np.sin(angBeta)],
                   [0,1,0],
                   [-np.sin(angBeta), 0, np.cos(angBeta)]])
    Rz = np.array([[np.cos(angGamma), -np.sin(angGamma), 0],
                   [np.sin(angGamma), np.cos(angGamma), 0],
                   [0,0,1]])
    M[:3,:3] = Rz @ Ry @ Rx
    glMultMatrixf(M.T)
...
def key_callback(window, key, scancode, action, mods):
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        if key==glfw.KEY_1:
            ...
            angAlpha = angAlpha + np.radians(10)
        elif key == glfw.KEY_Z:
            angAlpha = angAlpha - np.radians(10)
        elif key == glfw.KEY_S:
            angBeta = angBeta + np.radians(10)
        elif key == glfw.KEY_X:
            angBeta = angBeta - np.radians(10)
        elif key == glfw.KEY_D:
            angGamma = angGamma + np.radians(10)
        elif key == glfw.KEY_C:
            angGamma = angGamma - np.radians(10)
        elif key == glfw.KEY_V:
            angAlpha = np.radians(0)
